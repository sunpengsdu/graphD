...
template <class VertexT, class AggregatorT = DummyAgg>
class Comper
{
public:
    ...
    Comper()
	{
		...
		//---
		if(type_index(typeid(AggregatorT)) != type_index(typeid(DummyAgg)))
		{
    	...
    	if(msg_recved)
    	{
			...
			for (int i = 0; i < vertexes.size(); i++) {
				...
				if(vertexes[i].active || msgs.size() > 0)
				{
					...
					if(global_aggregator_used) aggregator.stepPartial(&vertexes[i]);
	...
	*/
    ...
    void agg_sync() //implementation to avoid bug of MPICH-3.1.3 MPI_Bcast, send through COMM_CHANNEL_100 (see communication.h)
	{
		if (_my_rank != MASTER_RANK) { //send partialT to aggregator
			//gathering PartialT
			PartialT* part = aggregator.finishPartial();
