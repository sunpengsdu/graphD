...
vector<int *> batch_comp_step; //batch_comp_step[s] = batch_comp[] for superstep s
...
int rstep_before; //written by recv-er before recv-sync
int rstep_after; //written by recv-er after recv-sync
...
bool job_finished; //written by recv-er, set if termination condition holds, and threads will exit
//to update, please lock mtx_r2s (due to Sender.wait_for_receiver())
//-------------------------------------------------------------------------------------
//locks
...
//-------------------------------------------------------------------------------------
//if combiner is used, we need to enlarge MAX_MSG_STREAM_SIZE to at least max{|Vi|}
int * global_vsize; //global_vsize[i] = number of vertices on worker i
void gather_vsizes(int my_size)
{
    MPI_Allgather(&my_size, 1, MPI_INT, global_vsize, 1, MPI_INT, MPI_COMM_WORLD);
}
//====================================================================================
...
*/
vector<int> active_vnum_step; //written by Comper, read by Recver
...
//---
int global_max_vnum; //maximum number of vertices on any machine
